...
def get_taylor_data(symbol):
    ...
    # Renaming outputs to logical chart boundaries
...
try:
    ...
    # --- METRICS DISPLAY (SIMPLIFIED LABELS) ---
    ...
    # --- HISTORICAL LEDGER ---
    st.markdown("---")
    st.subheader("📋 Historical Calculation Ledger")
    ledger_df = data[['Open', 'High', 'Low', 'Close', 'Envelope_High', 'Envelope_Low']].dropna()
    # Rename for dataframe look
    ledger_df.columns = ['Open', 'High', 'Low', 'Close', 'Projected Envelope High', 'Projected Envelope Low']
